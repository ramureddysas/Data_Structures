...
setwd ("c:\\Users\\mouna\\Downloads")
asl <- read_sas("adsl.sas7bdat")

asl1 <- asl %>% 
  filter(SAFFL=="Y", !is.na(TRT01PN)) %>% group_by (TRT01PN)%>% 
  summarise(N=n(), .groups = "drop")

 dir.create("c:/Users/mouna/Rlibs1", recursive = TRUE)
 .libPaths("c:/Users/mouna/Rlibs1")
 
 my_data <- data.frame(id = 1:3, name = c("Alice", "Bob", "Charlie"))
 
 saveRDS(my_data, "my_dataset.rds")
 #loaded_data <- readRDS("my_dataset.rds")
